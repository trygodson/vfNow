...
const GenralElection = ({ items, indexs, user, setLoader, HomeFtn, bottom, setElectionID, setBusinesID }) => {
  const navigate = useNavigate();
  const { t } = useTranslation();

  const calculateTimeLeft = () => {
    if (items.election_date_time != '') {
      let diffTime = Math.abs(new Date().valueOf() - new Date(`${items.election_date_time}`).valueOf());
  ...
  const [ageLimitDiv, setAgeLimitDiv] = useState(items?.age_limitation == 0 ? false : true);
        ...
        <div class="prod-snip">
          ...
          >
            <div class="logo-sec">
              <img
                class="img-fluid"
                src={item?.business_details?.avatar ? item?.business_details?.avatar : logo}
                alt="logo"
              />
            </div>
            <div class="cont-sec">
              <h4 class="text-truncate text-uppercase">{item?.business_name}</h4>
          ...
          <div class="img-wrap">
            ...
            >
              <img
                src={item?.election_status == 'Not Started' ? nostart : startyellow}
                class={`  ${item?.election_status != 'Ended' ? '' : 'img-white'} `}
                alt=""
              />
              <span>
                {item?.election_status == 'Started' && <img class="ico" src={vote} alt="ico" />}
              ...
              >
                {item?.gift_images?.map((itemChild, index) => {
                  return (
                    ...
                    >
                      <img class="img-fluid" src={itemChild?.picture} alt="Thumbnail" />
                    </div>
                  );
                })}
                {item?.video != '' && (
            ...
            <div class="d-flex start flex-row justify-content-between">
              ...
              <div class="ico-cont justify-content-end">
                {item?.election_status == 'Not Started' ? (
                  ...
                  <>
                    <img src="./images/vote-ico.svg" className="icon-size" alt="ico" />
                    <strong>
                    ...
                    {item?.join_status == false ? (
                      user?.login_as == 'visitor' ? (
                        ...
                        <button
                          onClick={() => {
                            if (!ageLimitDiv) {
                              AddElectionCandidate(item?.election_id, item, indexs);
                            }
                          }}
                          class="bg-transparent border-0 link"
                        >
