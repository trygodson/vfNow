...
const ElectionBox = ({
  ...
  const calculateTimeLeft = () => {
    let diffTime = Math.abs(
      new Date().valueOf() - new Date(`${item.election_date_time}`).valueOf()
    );
  ...
  const [ageLimitDiv, setAgeLimitDiv] = useState(
    item.age_limitation == 0 ? false : true
  );
  ...
  return (
    <div class="prod-snip" key={index}>
      ...
      >
        <div
          class={`${
            item?.election_status == "Not Started" ? "status" : "status start"
          }`}
        >
          <img
            src={
              item?.election_status == "Not Started"
                ? "images/not-started-bg.svg"
                : "images/yellow-start-bg.svg"
            }
            alt=""
          />
          ...
          <div class="ico-cont justify-content-end">
            {item?.election_status == "Not Started" ? (
              <>
                ...
                <span>
                  {item?.candidates_count == "" ? "0" : item?.candidates_count}{" "}
        ...
        <div class="btm-sec">
          {item.join_status == false ? (
            user?.login_as == "visitor" ? (
              ...
              >
                <img
                  class="img-fluid text-uppercase me-2"
                  src="images/join-candidate-ico.svg"
                  alt="ico"
                />
              ...
              <button
                onClick={() => AddElectionCandidate(item.election_id, item)}
                class="bg-transparent border-0 link"
              >
