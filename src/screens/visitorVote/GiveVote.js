...
export default function GiveVote() {
  ...
  const calculateTimeLeft = () => {
    if (visitorvote) {
      let diffTime = Math.abs(new Date().valueOf() - new Date(`${visitorvote}`).valueOf());
      ...
      } else if (Math.floor(days) == 0 && Math.floor(hours) == 0) return minutes + 'm ' + secs + 's';
  ...
  function GiveVoteDetail(users) {
    ...
    ApiCall('Post', API.GiveVoteDetailApi, formData, {
      ...
      .then((resp) => {
        ...
        setVisitorVote(resp.data.data?.election_details?.vote_date_time);
        ...
        <>
          ...
          <section class="content-sec row">
            <div class="vote-sec px-0">
              <img class="w-100 img-fluid" src="images/vote-base-bg.jpg" alt="ico" />
              ...
              <div class="avatar-img">
                <img src={user?.avatar ? user?.avatar : 'images/avatar-big-1.png'} alt="img" />
              </div>
            </div>
            ...
            <div class="product-wrap item-snippet">
              ...
              {visitorvote && users?.login_as == 'visitor' ? (
                ...
                <>
                  {users?.login_as == 'visitor' ? (
                    ...
                    <button
                      class="w-100 btn btn-yellow text-uppercase mb-3"
                      onClick={() => VisitorVoteAdd(user?.candidate_id, user?.election_id)}
                    >
                      {t('Buttons.Confirm Vote')}
                    </button>
                  )}
              ...
              {visitorvote && (
                <p class="my-4 text-center">
                  <button onClick={() => navigate(-1)} class="transparent fw-bold border-0">
                    {t('Buttons.CONTINUE')}
                  </button>
                </p>
              )}
        ...
        <div class="modal-dialog modal-dialog-centered heigh-cal">
          <div class="modal-content minh-unset">
            ...
            <div class="button-btm-sec margin-set">
              <Link data-bs-dismiss="modal" to={'/login'} class="btn btn-yellow text-uppercase w-100">
                {t('Buttons.Sign-up')}
              </Link>
              <Link
                to={'/GiveVoteRank'}
