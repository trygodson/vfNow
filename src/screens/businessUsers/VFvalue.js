...
export default function Election() {
  ...
  const [performance, setPerformance] = useState(location?.state?.preview?.performance);
  ...
  function Calculate() {
    if (!vfValue || !percent || !amount) {
      ...
      ApiCall('Post', API.BusinessPurchaseCalculate, formData, {
        ...
        .catch((error) => {
          setLoading(false);
          console.log('Calculation erorr reponse', error);
          //   reject(error.response);
        })
        .then((resp) => {
          setCalculate(resp.data.data);
          setLoading(false);
          console.log('Calculation list', resp.data);
        });
    }
  }
    ...
    <div class="container-fluid">
      ...
      <section class="content-sec row yellow-bg with-gray pb-5">
        <div class="login-wrap election-form px-3">
          ...
          <div class="performance-sec">
            <div class="per-cont">
              <div class="big-circle">
                <img class="img-fluid" src="images/performance-circle-big-dotted.png" alt="ico" />
                <div class="cont" style={{ left: '28%' }}>
          ...
          <span class="rate-tag">
            {t('Payment.Rate applied')} {calculate?.rate_applied ? calculate?.rate_applied : '0.01'} SGD/VF
          </span>
          <div class="col-12 mt-4 text-center mb-5">
            <button disabled={loading} class="btn btn-gray px-4" style={{ width: '140px' }} onClick={() => Calculate()}>
              {loading ? (
